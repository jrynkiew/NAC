...
# CPPFLAGS = -I$(IMGUI_DIR) -I$(SRC_DIR)/backends -I$(NAC_DIR) -g -Wall -Wformat -pthread -Os -DIMGUI_DISABLE_FILE_FUNCTIONS $(EMS)
# LIBS += $(EMS)
# LDFLAGS += -s WASM=1 -s PTHREAD_POOL_SIZE=2 -pthread -s NO_EXIT_RUNTIME=0 -s ASSERTIONS=1 -s --shell-file shell_minimal.html --use-preload-plugins --preload-file $(NAC_DIR)/textures@/ --preload-file $(SRC_DIR)/configuration@/
